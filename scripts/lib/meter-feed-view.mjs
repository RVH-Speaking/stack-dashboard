...
export function renderMeter(text, { live = true, now = new Date(), fallback = false, refreshStatus } = {}) {
  ...
  const lanes = feed.lanes.map(lane => {
    ...
    const status = boundary ? 'VEROUDERD' : lane.freshness === 'CURRENT' ? live ? 'ACTUEEL' : unknown : lane.freshness;
  ...
  const cards = lanes.map(lane => `<article data-meter-lane="${lane.alias}" data-family="${family(lane.alias)}" data-status="${lane.status}" aria-labelledby="lane-${lane.alias}">
    ...
    <ul class="windows">${lane.windows.map(w => `<li><h4>${w.model_alias} <span>${w.window_alias}</span></h4><p class="quota">${w.quota_group} · gedeelde quota worden niet opgeteld</p>
      <div class="capacity"><strong>${w.remaining_percent === null ? unknown : `${w.remaining_percent}%`}</strong><span>resterend</span></div>
      ${w.remaining_percent === null ? '' : `<meter min="0" max="100" value="${w.remaining_percent}" aria-label="${lane.alias} ${w.model_alias} ${w.window_alias} resterend">${w.remaining_percent}%</meter>`}
      <dl>${detail('Reset (UTC)', time(w.reset_at))}${detail('Reset over', w.countdown_seconds === null ? unknown : `<span data-meter-countdown>${duration(w.countdown_seconds)}</span>`)}${detail('Abonnementsverlenging', time(w.subscription_renewal_at))}${detail('Creditverval', time(w.credit_expires_at))}</dl></li>`).join('') || `<li>${unknown}</li>`}</ul>
  ...
  const subscriptions = lanes.map(l => `<li><strong>${l.alias}</strong><dl>${l.windows.map(w => detail(`${w.model_alias} / ${w.window_alias} · verlenging`, time(w.subscription_renewal_at)) + detail('Creditverval', time(w.credit_expires_at))).join('') || detail('Abonnement / credits', unknown)}</dl></li>`).join('');
